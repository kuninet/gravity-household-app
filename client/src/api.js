...
export async function fetchPriceTrend({ keyword, mode = 'jev', from, to, selectedItems } = {}) {
  const params = new URLSearchParams()
  ...
  const res = await fetch(`${API_BASE}/price_trend?${params.toString()}`)
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
    throw new Error(err.error || '価格推移データの取得に失敗しました')
  }
  return res.json()
}
