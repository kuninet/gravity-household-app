...
const currentView = ref('dashboard') // 'dashboard' | 'analysis' | 'multi_year_analysis' | 'price_trend' | 'fixed_costs' | 'import' | 'receipt_check'
...
<template>
  <div class="min-h-screen bg-ground text-ink">
      ...
      <div class="max-w-screen-2xl mx-auto px-6 py-3.5">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div class="flex items-center gap-2.5">
            <div
              class="w-7 h-7 rounded-md bg-accent grid place-items-center text-white font-serif italic text-base leading-none"
            >
              G
            </div>
          ...
          <div class="flex items-center gap-2">
            <div
              ...
              class="flex items-center gap-1 pl-1.5 pr-2 py-[3px] border border-rule rounded-lg bg-surface"
            >
              <button
                @click="changeMonth(-1)"
                class="text-ink-2 hover:text-ink px-1.5 py-0.5 rounded transition"
                aria-label="前の月"
              >
                ‹
              </button>
              ...
              <span class="text-[10px] text-ink-3 font-mono ml-1.5 pl-1.5 border-l border-rule">{{
                rangeLabel
              }}</span>
            </div>
            <button
    ...
    <main class="max-w-screen-2xl mx-auto p-6">
      ...
      <div v-if="currentView === 'dashboard'" class="animate-fade-in">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div class="relative bg-surface border border-rule rounded-xl px-4 py-3.5">
            <h3 class="m-0 mb-1.5 text-[10px] tracking-[0.14em] uppercase text-ink-3 font-semibold">
              収入
            </h3>
            <span
              ...
              :class="
                deltaFor('income').diff >= 0 ? 'bg-pos-soft text-pos' : 'bg-neg-soft text-neg'
              "
              >{{ fmtPct(deltaFor('income').pct) }}</span
            >
            <p
              class="m-0 text-[22px] font-semibold text-pos font-mono font-tabular tracking-tight leading-none"
            >
              <span class="text-[13px] text-ink-3 font-medium mr-0.5">¥</span
              >{{ fmt(summary.total.income) }}
            </p>
            <div
              v-if="deltaFor('income')"
              class="mt-3 pt-2.5 border-t border-dashed border-rule flex justify-between text-[11px] text-ink-3 font-tabular"
            >
          ...
          <div class="relative bg-surface border border-rule rounded-xl px-4 py-3.5">
            ...
            <span
              ...
              class="absolute top-3.5 right-3.5 font-mono text-[10px] px-1.5 py-[2px] rounded"
              :class="
                deltaFor('expense').diff <= 0 ? 'bg-pos-soft text-pos' : 'bg-neg-soft text-neg'
              "
              >{{ fmtPct(deltaFor('expense').pct) }}</span
            >
            <p
              class="m-0 text-[22px] font-semibold text-neg font-mono font-tabular tracking-tight leading-none"
            >
              <span class="text-[13px] text-ink-3 font-medium mr-0.5">¥</span
              >{{ fmt(summary.total.expense) }}
            </p>
            <div
              v-if="deltaFor('expense')"
              class="mt-3 pt-2.5 border-t border-dashed border-rule flex justify-between text-[11px] text-ink-3 font-tabular"
            >
          ...
          <div class="relative bg-surface border border-rule rounded-xl px-4 py-3.5">
            ...
            >
            <p
              class="m-0 text-[22px] font-semibold font-mono font-tabular tracking-tight leading-none"
              :class="summary.total.balance >= 0 ? 'text-ink' : 'text-neg'"
            >
              <span class="text-[13px] text-ink-3 font-medium mr-0.5">¥</span
              >{{ fmt(summary.total.balance) }}
            </p>
            <div
              v-if="prevSummary"
              class="mt-3 pt-2.5 border-t border-dashed border-rule flex justify-between text-[11px] text-ink-3 font-tabular"
          ...
          <div class="bg-surface border border-rule rounded-xl p-4 md:row-span-2 md:col-start-4">
            <div class="text-[10px] tracking-[0.14em] uppercase text-ink-3 font-semibold mb-2">
              カテゴリ別支出
            </div>
          ...
          <div class="md:col-span-3">
            <TransactionForm
              ...
              @transaction-added="loadData"
              @prefill-applied="transactionPrefill = null"
            />
          </div>
        </div>
      ...
      <!-- Price Trend View -->
      <div v-else-if="currentView === 'price_trend'">
        <PriceTrendView />
      </div>
